...
func TestCreateLearning(t *testing.T) {
	...
	if got.Status != LearningStatusActive {
		t.Errorf("status = %q, want %q", got.Status, LearningStatusActive)
	}
	if len(got.Files) != 2 {
...
func TestUpdateLearningStatus(t *testing.T) {
	...
	if err := db.UpdateLearningStatus(learning.ID, LearningStatusStale); err != nil {
		t.Fatalf("failed to update status to stale: %v", err)
	}

	got, _ := db.GetLearning(learning.ID)
	if got.Status != LearningStatusStale {
		t.Errorf("status = %q, want %q", got.Status, LearningStatusStale)
	}

	// Transition: stale -> archived
	if err := db.UpdateLearningStatus(learning.ID, LearningStatusArchived); err != nil {
		t.Fatalf("failed to update status to archived: %v", err)
	}

	got, _ = db.GetLearning(learning.ID)
	if got.Status != LearningStatusArchived {
		t.Errorf("status = %q, want %q", got.Status, LearningStatusArchived)
	}
}
...
func TestGetLearningsByConcepts_ExcludesStale(t *testing.T) {
	...
	if learnings[0].Status != LearningStatusActive {
