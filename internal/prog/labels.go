...
func (db *DB) CreateLabel(l *Label) error {
	_, err := db.Exec(`
...
func (db *DB) GetLabelByName(project, name string) (*Label, error) {
	var l Label
	var color *string
	err := db.QueryRow(`
...
func (db *DB) GetLabel(id string) (*Label, error) {
	var l Label
	var color *string
	err := db.QueryRow(`
		SELECT id, name, project, color, created_at, updated_at
...
func (db *DB) ListLabels(project string) ([]Label, error) {
	rows, err := db.Query(`
		SELECT id, name, project, color, created_at, updated_at
		FROM labels
		WHERE project = ?
...
func (db *DB) EnsureLabel(project, name string) (*Label, error) {
...
func (db *DB) GetItemLabels(itemID string) ([]Label, error) {
	rows, err := db.Query(`
...
func (db *DB) PopulateItemLabels(items []Item) error {
	if len(items) == 0 {
		return nil
	}
