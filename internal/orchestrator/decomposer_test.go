...
func TestParseDecompositionResponse_Valid(t *testing.T) {
	response := `[
		{
			"title": "Task 1",
	...
	tasks, err := parseDecompositionResponse(response)
	if err != nil {
		t.Fatalf("parseDecompositionResponse failed: %v", err)
	}
	...
	if tasks[0].Description != "Description 1" {
		t.Errorf("Task 0 description = %q, want %q", tasks[0].Description, "Description 1")
	}
	if len(tasks[0].DependsOn) != 0 {
		t.Errorf("Task 0 should have no dependencies, got %d", len(tasks[0].DependsOn))
	}

	// Check second task - should depend on first
...
func TestParseDecompositionResponse_WithExtraText(t *testing.T) {
...
func TestParseDecompositionResponse_NoJSONArray(t *testing.T) {
	response := "No JSON here"

	_, err := parseDecompositionResponse(response)
	if err == nil {
...
func TestParseDecompositionResponse_InvalidJSON(t *testing.T) {
...
func TestParseDecompositionResponse_EmptyArray(t *testing.T) {
	response := "[]"
...
func TestParseDecompositionResponse_UnknownDependency(t *testing.T) {
	response := `[
		{
			"title": "Task 1",
			"description": "Description",
...
func TestParseDecompositionResponse_TaskFields(t *testing.T) {
	response := `[
		{
			"title": "Task Title",
...
func TestValidateNoCycles_MissingDependency(t *testing.T) {
	// Task depends on non-existent task - should not error (just skipped)
	tasks := []*models.Task{
		{ID: "1", Title: "Task 1", DependsOn: []string{"nonexistent"}},
	}

	// validateNoCycles only checks for cycles, not missing deps
	err := validateNoCycles(tasks)
	if err != nil {
...
func TestParseDecompositionResponse_MultipleDependencies(t *testing.T) {
	response := `[
		{
			"title": "Setup",
