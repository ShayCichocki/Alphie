...
func TestMergeResult_NeedsSemanticMerge(t *testing.T) {
	result := MergeResult{
		Success:            false,
...
func TestNewMergeHandler(t *testing.T) {
	handler := NewMergeHandler("session-abc", "/tmp/repo")
...
func TestMergeResult_DiffAndChangedFiles(t *testing.T) {
	// Test that diff and changed files are properly captured on success
	result := MergeResult{
		Success: true,
...
func TestMergeResult_MultipleConflictFiles(t *testing.T) {
	// Test handling of multiple conflict files
	conflicts := []string{
...
func TestMergeHandler_EmptyBranchName(t *testing.T) {
	// Test handling of edge case with empty branch names
	handler := NewMergeHandler("", "/tmp/repo")
