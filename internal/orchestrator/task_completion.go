...
func (o *Orchestrator) handleAbortedTask(task *models.Task, result *agent.ExecutionResult) {
	...
	// TODO: state persistence removed - o.updateTaskState(task)
	// TODO: state persistence removed - o.updateAgentState(result.AgentID, "failed")

	// Update prog task status
	// TODO: progCoord removed - o.progCoord.BlockTask(task.ID, failureMsg)
...
func (o *Orchestrator) handleSuccessfulTask(ctx context.Context, task *models.Task, result *agent.ExecutionResult) (*MergeOutcome, error) {
	...
	if o.merger != nil {
		...
		if o.mergeVerifier != nil && o.mergeVerifier.ShouldVerify() {
			// TODO: progCoord removed - o.progCoord.LogTask(task.ID, "Verifying merged code builds...")
			...
			if err != nil || !verifyResult.Passed {
				...
				// TODO: progCoord removed - o.progCoord.LogTask(task.ID, fmt.Sprintf("Build verification failed: %s", errorMsg))
				...
				// TODO: progCoord removed - o.progCoord.LogTask(task.ID, "Merge rolled back due to build failure")
			...
			// Verification passed
			// TODO: progCoord removed - o.progCoord.LogTask(task.ID, fmt.Sprintf("Build verification passed (%v)", verifyResult.Duration))
	...
	// TODO: state persistence removed - o.updateTaskState(task)
	// TODO: state persistence removed - o.updateAgentState(result.AgentID, "done")
	...
	// TODO: progCoord removed - o.progCoord.CompleteTask(task.ID)
...
func (o *Orchestrator) handleFailedTask(task *models.Task, result *agent.ExecutionResult) {
	...
	if shouldRetry {
		// TODO: progCoord removed - o.progCoord.LogTask(task.ID, fmt.Sprintf("Attempt %d failed: %s. Retrying...", task.ExecutionCount, result.Error))
	} else {
		// TODO: progCoord removed - o.progCoord.BlockTask(task.ID, result.Error)
	}
...
func (o *Orchestrator) performMerge(ctx context.Context, taskID string, result *agent.ExecutionResult) (*MergeOutcome, error) {
	...
	// TODO: progCoord removed - o.progCoord.LogTask(taskID, "Starting merge operation (queued)")
	...
	case outcome := <-resultCh:
		if outcome.Success {
			// TODO: progCoord removed - reinstate if needed
		...
		// Merge failed
		// TODO: progCoord removed - o.progCoord.LogTask(taskID, fmt.Sprintf("Merge failed: %s", outcome.Reason))
